...
cp /usr/lib/x86_64-linux-gnu/libopenal.so.1 "${OUTPUT}/libopenal.so.1"
...
make && make install
cp output/lib/libSDL2.so "${OUTPUT}/libSDL2-2.0.so.0"
popd
...
make linux
cp src/liblua.so.5.1 "${OUTPUT}/liblua5.1.so.0"
popd
